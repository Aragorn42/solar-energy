...
from models import Transformer, DLinear, PatchTST, iTransformer
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                B, T, C = pred.shape
                pred_inv = test_data.inverse_transform(
                    pred.reshape(B * T, C)
                ).reshape(B, T, C)

                true_inv = test_data.inverse_transform(
                    true.reshape(B * T, C)
                ).reshape(B, T, C)
                ...
                #     pdd = np.concatenate((input[0, :, -1], pred[0, :, -1]), axis=0)
                #     # visual(gt, pdd, os.path.join(folder_path, str(i) + '.pdf'))

        if self.args.test_flop: # default False
        ...
        print('1-rmse/cap:{}, 1-mae/cap:{}'.format(1-rmse/self.args.cap, 1-mae/self.args.cap))
