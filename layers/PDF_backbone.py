...
class PDF_backbone(nn.Module):
    def __init__(self, c_in: int, context_window: int, target_window: int,
        ...
        self.backbone = nn.ModuleList([nn.Sequential(
            TSTiEncoder(c_in, patch_num=token, patch_list=self.dim_list[i], max_seq_len=max_seq_len,
...
class TSTiEncoder(nn.Module):  # i means channel-independent
    def __init__(self, c_in, patch_num, patch_list, max_seq_len=1024,
        ...
        # Input encoding
        q_len = patch_num
        self.W_P = nn.Linear(patch_list, d_model)  # Eq 1: projection of feature vectors onto a d-dim vector space
        self.seq_len = q_len
