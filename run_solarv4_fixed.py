...
print("[hybrid]  混合模型策略：短临PatchTST + 超短期DLinear + 短期NLinear")
...
OUTPUT_DIR = r"/home/zhaopp/workspace/Hyper/output"
...
def era5_resample(era5_df, freq):
    ...
    return era5_df.resample("1h").mean().ffill().ffill()
...
def compute_metrics(pred_norm, true_norm, cs_ghi_arr, eval_idx,
    ...
    # 第 i 个样本的预测目标时间是 test_ts[i + seq_len + eval_idx]
    n = min(len(pred_power), len(test_ts) - seq_len - eval_idx)
    ts = test_ts[seq_len + eval_idx:seq_len + eval_idx + n]
...
def ensemble_predict_and_eval(df, task_cfg, cap, model_key, model_paths, model_type='dlinear'):
    ...
    tr_ld, val_ld, te_ld, mean, std, power_kt_idx, power_idx, cs_ghi_idx, predict_power_kt = \
        make_loaders(df, seq_len, pred_len)
    if len(te_ld) == 0:
        print(f"      [跳过] 测试集窗口不足: test_batches={len(te_ld)}")
        return None
    models = []
...
csv_path = os.path.join(OUTPUT_DIR, "results_hybrid.csv")
...
plt.tight_layout()
fig_path = os.path.join(OUTPUT_DIR, "results_table.png")
