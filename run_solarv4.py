...
GUOWANG_DIR = r"F:\3datas\国网\solar_stations\csv_single"
SKIPPD_DIR  = r"F:\3datas\SKIPPD"
GEFCOM_DIR  = r"F:\3datas\GEFCom2014\Solar"
ERA5_DIR    = r"F:\3datas\ERA5"
OUTPUT_DIR  = r"F:\3datas\results"
...
def add_lag_rolling(df, freq="15T"):
    df   = df.copy()
    sph  = 4 if freq == "15T" else 1
...
def add_time_features(df, freq="15T"):
...
def era5_resample(era5_df, freq):
    if freq == "15T":
        idx = pd.date_range(era5_df.index[0], era5_df.index[-1], freq="15T")
...
def load_guowang(site_num):
    ...
    return df.resample("15T").mean().ffill().bfill()
...
def load_skippd():
    ...
    df["power"] = df["power"].clip(lower=0)
    if len(df) > 150000: df = df.resample("15T").mean()
    return df.ffill().bfill()
...
def prepare(raw_df, era5_df, freq="15T", lat=None, lon=None):
    df = raw_df.copy()
    df.index = strip_tz(df.index)
...
if RUN_GUOWANG:
    ...
    for sn, mw in SITES.items():
        ...
        df = prepare(raw, load_era5_csv(E5MAP[sn]), freq="15T", lat=lat, lon=lon)
        cap = CAPS[f"site{sn}"]
